...
int PmergeMe::getVCount() { return _vCount; }
...
std::list<t_data*>::iterator PmergeMe::getIteratorAtIdx(int index) {
...
void PmergeMe::insertList(int i) {
	...
	std::list<t_data*>::iterator right_it = getIteratorAtIdx(i);

	t_data* value_to_insert = (*getIteratorAtIdx(i))->left;
	...
	t_data* tmp = 0;
	std::list<t_data*>::iterator next_it = getIteratorAtIdx(i + 1);
    tmp = *next_it;
...
void PmergeMe::sortList() {
	...
	while (!flag) {
		...
		for (int l = 0; l < jNumGap && k >= 0; k--) {
			std::list<t_data*>::iterator it = getIteratorAtIdx(k);
